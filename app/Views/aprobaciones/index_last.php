<?php echo view('_partials/header'); ?>
  ...
  <script src="<?= base_url('js/_axios.js') ?>"></script>
        ...
        <div class="col-l bg-grayMid rounded border border-grayMid p-2">
          <form id="form_search" class="flex flex-col space-y-4 w-full items-center justify-center px-6 " method="post" >
            ...
            <div class="relative flex flex-col  w-full">
              <h5 class="text-center uppercase">COSTOS</h5>
            ...
            <div class="relative flex flex-col  w-full">
              <h5 class="text-center uppercase">Desarrollo</h5>
            ...
            <div class="relative flex flex-col  w-full">
              <h5 class="text-center uppercase">Calidad</h5>
...
form_aprobacion_desarrollo?.addEventListener('submit', e => {
  ...
  if (comentario.value.length > 5) {
    ...
    .then( r => {
      ...
      // return;
      e.target.reset();
      initModalAprob(r.data.art_id, r.data.area);
      loadAllArticles()
      btn.disabled = false;
    })
...
form_aprobacion_calidad?.addEventListener('submit', e => {
...
form_aprobacion_costos?.addEventListener('submit', e => {
...
const trimFileName = (fileName, maxLength = 18) => {
  ...
  const initModalAprob = (id, aprob_type) => {
    ...
    Cotizador.get('/get_aprobacion', { params: { art_id: id, area: aprob_type } })
    .then( r => {
      console.log(r.data)
      ...
      if (r.data) {
        // r.data.forEach(aprob => {
          ...
          div.classList.add('row-comentario');
          div.innerHTML = `
            <div class="w-64 p-2">${r.data.name} ${r.data.last_name}</div>
            <div class="w-28 p-2 text-center">${dateToString(r.data.created_at)}</div>
            <div class="w-28 p-2 text-center font-bold ${setArticleStatus(r.data.status).color}">${r.data.status}</div>
            <div class="p-2">${r.data.comentario}</div>
          `;
        ...
        aprobId.value = r.data.id;
        // console.log(comentario)
        comentario.value = r.data.comentario;
        // console.log(document.querySelector(`#status_${aprob_type}`))
        setSelectedOption(`#status_${aprob_type}`, r.data.status, 'string');
  ...
  const initRowBtn = () => {
    ...
    allBtnOpen?.forEach( (btn, index) => {
      ...
      btn.addEventListener('click', e => {
        ...
        if (modal_id == 'modal_edit') {
        ...
        } else if (modal_id == 'modal_aprobacion_costos') {
        ...
        } else if (modal_id == 'modal_aprobacion_desarrollo') {
          ...
          initModalAprob(id, aprob_type);
        } else if (modal_id == 'modal_aprobacion_calidad') {
  ...
  // initRowBtn(); // usar esto solo cuando carga usando PHP los datos en tabla
