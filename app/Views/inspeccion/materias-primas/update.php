				...
				<div class="general__section">
					...
					<div class="general__col border-r ">
						<div class="col header border-l border-b w-full lg:w-[25%]">
							...
							<input type="text" data-field="NOMBRE DE TRANSPORTISTA" class="mobile-input lg:hidden h-8 text-gray">
						</div>
				...
				<div class="etiqueta__section">
					...
					<div class="file__section border-t border-l  ">
						...
						<div class="file__section--title header-b border-b border-r  ">
							...
							<input type="hidden" name="etiqueta[id]" value="<?= esc($etiq['id']) ?>">
							<input type="hidden" name="etiqueta[observacion]">
							<input type="hidden" name="etiqueta[aprobado]">
						</div>
						...
						<div class="file__section--value border-r border-t border-b">
							<label class="label--check">
								<span>Aprobado</span>
								<input type="radio" data-id="<?= esc($etiqueta['id']) ?>" name="items[<?= esc($etiqueta['id']) ?>][check]" data-value="si" class="checkbox_inspec_edit hidden">
								<span class="checkbox-label-si"><i class="fas fa-check "></i></span>
							</label>

							<label class="label--check">
								<span>Rechazado</span>
								<input type="radio" data-id="<?= esc($etiqueta['id']) ?>" name="items[<?= esc($etiqueta['id']) ?>][check]" data-value="no" class="checkbox_inspec_edit hidden" >	
								<span class="checkbox-label-no"><i class="fas fa-x "></i></span>
							</label>
						</div>
		...
		btn?.addEventListener('click', (e) => {
			...
			if (img === "img_etiqueta") {
				...
				.then(r => {
					...
					const etiq = document.querySelector(`input[name="etiqueta[observacion]"`);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  inputs.forEach(input => {
    ...
    if (item) {
      ...
      input.dataset.id = item.id; // 🔁 Add data-id attribute for backend reference
    }
  });
});
...
const toggleCheckboxEdit = (e) => {
	let _val = e.target.getAttribute('data-value');
	let id = e.target.getAttribute('data-id');
	document.querySelector(`input[name="etiqueta[aprobado]"]`).value = _val;
	console.log(_val);
};
...
const allCheckboxToggleEdit = document.querySelectorAll('.checkbox_inspec_edit');
allCheckboxToggleEdit?.forEach(input => {
	input.addEventListener('change', toggleCheckboxEdit);
});
...
btn_submit?.addEventListener('click', e => {
	...
	let form = document.querySelector("#form_edit_inspeccion");
	const formData = new FormData(form);

	// Detect screen type
	const isMobile = window.innerWidth <= 1024;

	// Collect all unique data-field labels
	const inputs = document.querySelectorAll('input[data-field]');
	const handledIds = new Set();

	inputs.forEach(input => {
		const label = input.dataset.field?.trim();
		if (!label) return;

		// Get all inputs with same data-field (mobile + desktop versions)
		const group = Array.from(document.querySelectorAll(`input[data-field="${label}"]`));

		// Choose the correct input based on screen
		const correctInput = group.find(i => {
			const isMobileInput = i.classList.contains('lg:hidden');
			return isMobile ? isMobileInput : !isMobileInput;
		});

		if (correctInput && correctInput.dataset.id && !handledIds.has(correctInput.dataset.id)) {
			const id = correctInput.dataset.id;
			formData.append(`general_items[${id}][observacion]`, correctInput.value);
			formData.append(`general_items[${id}][id]`, id);
			handledIds.add(id);
		}
	});
	...
	Service.exec('post', `inspeccion/materias-primasu`, formData_header, formData)
	.then(r => {
		if(r.success){
