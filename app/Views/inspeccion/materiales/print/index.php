...
<head>
  ...
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.2/css/all.min.css" integrity="sha512-1sCRPdkRXhBV2PBLUdRb4tMg1w2YPf37qatUFeS7zlBy7jJI8Lf4VHwWfZZfpXtYSLy85pkm9GaYVYMfw5BC1A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
	...
	<link rel="stylesheet" href="<?= load_asset('_partials/print_insp_materiales.css') ?>">
...
<body>
	...
	<div class="wrapper">
		...
		<div class="header">
			...
			<a href="<?= base_url('inspeccion/lista/materiales') ?>" class="arrow-link">
			...
			<div class=" page__container">
				...
				<div class="general__section">
					...
					<div class="row-2 ">
						...
						<div>
							<input id="input_material" type="text" data-field="MATERIAL">
						</div>
						<div>
							<input type="text" data-field="PRESENTACION DEL EMBALAJE">
						</div>
						<div>
							<input type="text" data-field="CANTIDAD (Paquetes)">
						</div>
						<div>
							<input type="text" data-field="CANTIDAD TOTAL (Piezas)">
						</div>
					</div>
					...
					<div class="row-5 ">
						...
						<div>
							<input type="text" data-field="NOMBRE DEL TRANSPORTISTA" class="text-xs">
						</div>
							...
							<div class="section__items">
								<div class="resultados-container">
									...
									<?php if (in_array($item['item_number'], $items_espec_page1, true)): ?> 
										...
										<div class="resultados-block">

											<?php if (in_array($item['item_number'], $single_resultado, true)): ?> 
												<input type="text" name="detalles[resultados][<?= esc($item['id']) ?>]">
											<?php else: ?> 
												<input type="text" name="detalles[resultados][<?= esc($item['id']) ?>]">
												<input type="text" name="detalles[resultados][<?= esc($item['id']) ?>]">
												<input type="text" name="detalles[resultados][<?= esc($item['id']) ?>]">
									...
									<?php if (in_array($item['item_number'], ["4.4"], true)): ?> 
										<div class="resultados-title"><span>RESULTADO</span></div>
									<?php endif; ?>
								...
								<?php if (in_array($item['item_number'], $items_espec_clear, true)): ?> 
									<div class="item--alt ">
										<span data-id="<?= esc($item['id']) ?>">N.A.</span>
...
Service.setLoading();
		...
		const loadItems = () => {
			...
			registros.forEach(reg => {
				...
				// console.log(itemId, reg.aprobado);
				...
				if (observInput) {
					observInput.value = reg.observacion || 'N.A.';
				}
		...
		const loadChecks = () => {
			...
			detalles.forEach(reg => {
				...
				let resultados = [];
				try {
					resultados = JSON.parse(reg.resultado);
				} catch (e) {
					resultados = ['N.A.'];
				}

				// Select ALL inputs with the same name
				const inputs = document.querySelectorAll(
					`input[name="detalles[resultados][${itemId}]"]`
				);
				...
				inputs.forEach((input, index) => {
					...
					if (resultados[index] === 'si') {
						resultados[index] = '✓';
					} else if (resultados[index] === 'no') {
						resultados[index] = 'N.A.';
					}
					input.value = resultados[index] || 'N.A.';
				});
			});
			...
			registros.forEach(reg => {
				...
				// if(reg.aprobado !== 'si' && reg.aprobado !== 'no') {
				if(!reg.aprobado) {
					const all_radio = document.querySelectorAll(`input[data-id="${itemId}"]`);
					if (all_radio) {
						all_radio.forEach(s => {
					...
					const all_span = document.querySelectorAll(`span[data-id="${itemId}"]`);
					if (all_span) {
						// console.log(all_span)
						all_span.forEach(s => s.style.display = 'none');
					}
				}
			});
    ...
    document.getElementById('generate-pdf').addEventListener('click', () => {
      ...
      const element = document.querySelector('.pdf-container--print');
			...
			let materia_prima = document.querySelector('input[data-field="MATERIAL"]');
