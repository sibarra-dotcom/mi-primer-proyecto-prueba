  	...
  	<?php echo view('produccion/_partials/navbar'); ?>
			...
			<div class=" flex gap-4 items-center absolute right-4 lg:right-10 top-1/2 -translate-y-1/2">
				...
				<div class="flex w-fit items-center text-gray gap-x-2 text-sm">
					...
					<select id="order_by" class="select_order w-44">
						...
						<option value="asc-created_at">Fecha Creacion - a +</option>
						<option value="desc-created_at">Fecha Creacion + a -</option>
						<option value="asc-avance">% avance + a -</option>
						<option value="asc-avance">% avance - a +</option>
						<option value="desc-piezas">+ cantidad piezas faltantes</option>
						<option value="desc-piezas">- cantidad piezas faltantes</option>
					</select>
				</div>
		...
		<div class="w-full flex gap-4 items-start justify-center text-sm text-gray pl-4 pr-2" >
			...
			<div class="col-left w-56 bg-grayMid rounded border border-grayMid p-2">
  			<?php echo view('produccion/_partials/search_ordenes'); ?>
			</div>
			...
			`
				...
				<td>
					<div class="flex items-center gap-2 ">
						<a href="${root}/produccion/registro_diario/${orden.num_orden}" class=" hover:text-link">
							<i class="fas fa-plus text-lg pr-1"></i>
							<i class="fas fa-file-alt text-lg"></i>
						</a>
						<a href="${root}/produccion/ordenes_update/${orden.id}" class=" hover:text-warning">
							<i class="fas fa-pencil text-lg"></i>
						</a>
						<a href="${root}/produccion/ordenes_registros/${orden.num_orden}" class=" hover:text-icon">
...
// al usar SearchOrder no es necesario cargar las filas aqui
// loadRows();

	SearchOrder.init(['tabla-lista-orden-fab', 'order_by', 'form_search', '/produccion/search_lista_ordenes'], [
		['string', 'num_orden'],
		...
		['string', 'lote'],
		['date', 'created_at'],
		['date', 'fecha_primer_reporte'],
		['date', 'fecha_ultimo_reporte']
	]);
