...
<script>
  ...
  Service.setLoading();
	...
	const ordenId = document.querySelector('input[name="id"]');
  ...
  const loadRows = () => {
    Service.exec('get', `produccion/all_ordenes/${ordenId.value}`)
    .then(r => {
			Object.entries(r).forEach(([key, value]) => {
				let input;

				if (key == "fecha_compromiso") {
					let fecha_comp = document.querySelector('input[data-name="fecha_compromiso"]');
					let fecha_comp_hidden = document.querySelector('input[name="fecha_compromiso"]');
					fecha_comp.value = dateToString(value);
					fecha_comp_hidden.value = dateToString(value);
				}
